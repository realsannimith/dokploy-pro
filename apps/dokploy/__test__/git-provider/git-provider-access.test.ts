...
describe("getAccessibleGitProviderIds", () => {
	...
	describe("member access", () => {
		...
		it("can access a private provider explicitly assigned to them", async () => {
			const ids = await getAccessibleGitProviderIds(session(USER_MEMBER));
			expect(ids.has(providerPrivate.gitProviderId)).toBe(true);
		});
	...
	describe("member assignment boundaries", () => {
		...
		it("cannot access provider of another member without an assignment", async () => {
			mockDb.query.member.findFirst.mockResolvedValue({
				role: "member",
				accessedGitProviders: [],
			});
...
describe("canEditDeployGitSource", () => {
	...
	describe("member", () => {
		...
		it("cannot edit deploy using owner's private provider even when assigned", async () => {
			// Assignment permits using the provider for a new deploy, but it does not
			// permit changing an existing deploy backed by someone else's private provider.
			mockDb.query.gitProvider.findFirst.mockResolvedValue({
				userId: USER_OWNER,
				sharedWithOrganization: false,
			});
